...
const FieldsPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    const unsub = adminApi.subscribeFields(
      (data) => {
        setFields(data);
        setLoading(false);
      },
  ...
  const handleCreate = async (e) => {
    ...
    setError('');
    try {
      await adminApi.createField({ name: name.trim() }, admin?.email);
      setName('');
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const toggleActive = async (field) => {
    ...
    try {
      await adminApi.updateField(field.id, { isActive: !field.isActive }, admin?.email);
    } catch (err) {
      setError(err.message);
    }
  };
    ...
    <div className="tp-scope max-w-5xl space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
          <Tags className="text-brand" size={24} /> Interested Fields
        </h1>
        <p className="text-sm text-slate-500 font-medium mt-1">
          This master list feeds the "Fields of Interest" multi-select on the student registration form.
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="w-full md:w-80 shrink-0 rounded-2xl border border-slate-100 bg-white p-6 shadow-xl shadow-slate-100/40 space-y-4">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Add New Field</h3>
        ...
        {/* Right Side: Data Table List Card */}
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                ...
                <tbody className="divide-y divide-slate-50">
                  {fields.map((f) => (
                    <tr key={f.id} className="hover:bg-slate-50/40 transition-colors border-t border-slate-100">
                      <td className="px-6 py-4 font-bold text-slate-800">{f.name}</td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <StatusToggle checked={f.isActive} onChange={() => toggleActive(f)} />
                          <button
                            type="button"
      ...
      {fieldToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl text-center space-y-4 animate-fade-in animate-scale-up">
            ...
            <div>
              <h3 className="text-lg font-bold text-slate-900">Delete Field?</h3>
              <p className="mt-2 text-sm text-slate-500 font-medium leading-relaxed">
                Are you sure you want to delete <strong className="text-slate-800">"{fieldToDelete.name}"</strong>? This will remove it from candidate interest selections. This action cannot be undone.
              </p>
            </div>
            <div className="flex items-center justify-center gap-3 pt-2">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    await adminApi.deleteField(targetId, admin?.email);
                  } catch (err) {
                    setError(err.message);
                  }
                }}
